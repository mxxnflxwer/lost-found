...
async function main() {
  ...
  // We keep users but we'll recreate specific ones for the demo
  // To avoid unique constraint errors if users with these emails already exist, 
  // we'll delete them first or update them. For demo purposes, let's delete existing demo users.
  const demoEmails = [
  ...
  const items = [
    // --- MATCHED CASE 1: Backpack (Rajesh <-> Karthik) ---
    {
    ...
    // --- MATCHED CASE 2: iPhone (Priya <-> Aravind) ---
    {
    ...
    // --- MATCHED CASE 3: ID Card (Divya <-> Suresh) ---
    {
    ...
    // --- OTHER ITEMS linked to remaining users ---
    {
  ...
  console.log("Demo items created. Linking matching cases into Match table...");
  ...
  if (backpackLost && backpackFound) {
    await prisma.match.create({
      data: {
        lostItemId: backpackLost.id,
        foundItemId: backpackFound.id,
        score: 95,
        status: "APPROVED"
      }
    });
  ...
  if (idLost && idFound) {
    await prisma.match.create({
      data: {
        lostItemId: idLost.id,
        foundItemId: idFound.id,
        score: 90,
        status: "PENDING"
      }
    });
  ...
  if (phoneLost && phoneFound) {
    await prisma.match.create({
      data: {
        lostItemId: phoneLost.id,
        foundItemId: phoneFound.id,
        score: 100,
        status: "RESOLVED"
      }
    });
  ...
  console.log("✅ 11 users and 16 items seeded successfully with logical matches!");
}
